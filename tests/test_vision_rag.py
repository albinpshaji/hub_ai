...
from app.services.vector_service import store_image_vectors
...
async def test_store_image_vectors():
    ...
    with patch("app.services.vision_service.describe_image", AsyncMock(return_value="Detailed flowchart description")) as mock_desc, \
         patch("app.services.vector_service.get_ollama_embedding", AsyncMock(return_value=[0.1] * 768)) as mock_embed, \
         patch("app.services.vector_service.qdrant_client.upsert", AsyncMock()) as mock_upsert:
